...
@section('title', 'Tambah Pemilih Manual')
@section('header_title', 'Tambah Pemilih (DPT) Manual')

@section('content')
<div class="max-w-xl mx-auto space-y-6" x-data="{ category: '{{ old('category', 'siswa') }}' }">
    <div class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm">
        <div class="mb-6 pb-4 border-b border-slate-100 flex items-center justify-between">
            <div>
                <h2 class="text-base font-bold text-slate-900">Formulir Tambah Pemilih</h2>
                <p class="text-xs text-slate-500">Kode token akan dibuat secara acak dan otomatis oleh sistem</p>
            </div>
        ...
        <form action="{{ route('admin.voters.store') }}" method="POST" class="space-y-5">
            @csrf
